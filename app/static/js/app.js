...
const App = {
        ...
        if (menuButtons.length && menu) {
            menuButtons.forEach(menuButton => {
                menuButton.addEventListener('click', () => {
                    ...
                    menuButton.innerHTML = isOpen ? '<span>&#9776;</span>' : '<span>&times;</span>';
                });
            });
        }
    },
    ...
    setupHeaderScroll() {
        ...
        const shrinkHeader = () => {
            ...
            let shouldShrink = (!lastShrinkState && scrollY > 50)
                || (lastShrinkState && scrollY < 30)
                ? !lastShrinkState
                : lastShrinkState;
    ...
    initBuscaMobile() {
        ...
        if (botaoAbrir && botaoFechar && campoBusca) {
            botaoAbrir.addEventListener('click', function () {
                ...
                document.getElementById('header-direita-desktop').style.display = 'none';
    ...
    dashboardContratosCard() {
        ...
        fetch("/dashboard/contratos")
            ...
            .catch(err => {
                container.innerHTML = '<div class="text-danger">Erro ao carregar dados</div>';
                console.error("Erro ao buscar contratos:", err);
            });
    },

    dashboardContratosPorExercicioCard() {
        const container = document.getElementById("card-contratos-exercicio-container");
        if (!container) return;
        ...
        fetch("/dashboard/contratos-por-exercicio")
            ...
            .then(data => {
                ...
                const chartDom = document.getElementById("grafico-contratos-por-exercicio");
                if (!chartDom) return;
                ...
                chart.setOption({
                    tooltip: {
                        ...
                        formatter: p => `${p[0].axisValue}<br/><strong>${p[0].data} Contratos</strong>`
                    },
                    grid: {
                        right: 20
                    },
    ...
    dashboardRepresentacaoAnualValores() {
        const container = document.getElementById("card-representacao-anual-valores");
        if (!container) return;
        ...
        fetch("/dashboard/valores-por-exercicio")
            ...
            .then(data => {
                ...
                const chartDom = document.getElementById("grafico-representacao-anual-valores");
                if (!chartDom) return;
                ...
                chart.setOption({
                    tooltip: {
                        trigger: 'axis',
                        axisPointer: { type: 'shadow' },
                        formatter: p => `${p[0].axisValue}<br/><strong>R$ ${p[0].data.toLocaleString()}</strong>`
                    },
                    grid: { right: 20 },
                    xAxis: {
                        type: 'category',
    ...
    dashboardProximasAtividades() {
        ...
        fetch("/dashboard/atividades")
            ...
            .then(data => {
                ...
                const conteudo = atividades.slice(0, 25).map(atividade => `
            ...
            .catch(err => {
                ...
                container.innerHTML = '<div class="text-danger">Erro ao carregar atividades</div>';
            });
    },
    ...
    renderDashboardCardContratos({
        titulo = '', subtitulo = '', total = '', labelTotal = '',
        vigentes = 0, finalizados = 0, criticos = 0,
        dias120 = 0, dias90 = 0, dias45 = 0, outros = 0,
        icone = '/static/images/doc2.png'
    }) {
        return `
    ...
    renderDashboardCardContratosPorExercicio({ id, titulo, subtitulo, icone = '/static/images/doc2.png' }) {
        return `
            <div class="col-12 col-lg-3">
            ...
            .then(data => {
                ...
                data.usuarios.forEach(u => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td><span class="badge ${u.ativo ? 'success' : 'danger'}">${u.ativo ? 'Sim' : 'Não'}</span></td>
                        <td>
                            ...
                            <form method="post" action="/admin/usuarios/excluir/${u.id}" style="display:inline" onsubmit="return confirm('Confirma exclusão?')">
